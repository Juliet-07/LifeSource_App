...
export class HospitalService {
  ...
  async getDashboard(userId: string) {
    ...
    const [
      ...
      // Donation stats (30-day)
      donationsThisMonth,
      donationsByType,
    ] = await Promise.all([
      // Total available blood units
      this.inventoryModel.aggregate([
        { $match: { hospitalId, status: InventoryStatus.AVAILABLE } },
      ...
      this.inventoryModel.countDocuments({
        ...
        expiryDate: { $gt: new Date(), $lte: addDays(new Date(), 7) },
      }),

      // Expired (not yet cleaned up)
      this.inventoryModel.countDocuments({
        hospitalId,
        status: InventoryStatus.EXPIRED,
      }),

      // Total requests assigned to this hospital
      this.requestModel.countDocuments({ hospitalId }),

      // Pending/active requests
      this.requestModel.countDocuments({
        hospitalId,
        status: {
          $in: ['pending', 'notified_donors', 'confirmed_by_hospital'],
        },
      }),
      ...
      // Critical + high urgency open requests
      this.requestModel.countDocuments({
        hospitalId,
        urgency: { $in: ['critical', 'high'] },
        status: { $in: ['pending', 'notified_donors'] },
      }),
      ...
      this.requestModel
        ...
        .limit(5)
        .select(
          'bloodType urgency status unitsNeeded unitsFulfilled createdAt requestSource',
        )
        .lean(),
      ...
      this.appointmentModel.countDocuments({
        hospitalId,
        scheduledAt: { $gte: tomorrow, $lt: addDays(tomorrow, 6) },
        status: {
          $in: [AppointmentStatus.SCHEDULED, AppointmentStatus.CONFIRMED],
        },
      }),

      // Appointments awaiting hospital confirmation
      this.appointmentModel.countDocuments({
        hospitalId,
        status: AppointmentStatus.SCHEDULED,
      }),

      // Donations processed this month
      this.inventoryModel.countDocuments({
        hospitalId,
        status: { $in: [InventoryStatus.USED, InventoryStatus.AVAILABLE] },
        collectionDate: { $gte: thirtyDaysAgo },
      }),

      // Donations by type this month
      this.inventoryModel.aggregate([
        {
          $match: {
            hospitalId,
    ...
    const availableUnits = totalInventoryUnits[0]?.total || 0;
    ...
    return {
      data: {
        ...
        inventory: {
          ...
          expiringInSevenDays: expiringInWeek,
          expiredUnits: expiredCount,
        },
        requests: {
          total: totalRequests,
          pending: pendingRequests,
          fulfilledThisMonth: fulfilledRequests,
          urgentOpen: urgentRequests,
          fulfillmentRate,
          recent: recentRequests,
        },
        appointments: {
          today: todayAppointments,
          upcomingThisWeek: upcomingAppointments,
          awaitingConfirmation: pendingConfirmAppointments,
        },
        donations: {
          processedThisMonth: donationsThisMonth,
